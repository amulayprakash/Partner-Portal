...
//   const [first, setfirst] = useState(second)
  ...
  useEffect(() => {
    const getNewData=async(curPage) => {
        setLoading(true)
        try {
            await axios.get(`https://we-safe-partner-portal-backend1.onrender.com/admin?page=${curPage}&limit=10`).then(res=>{
    ...
    useEffect(() => {
        const getAllPartners=async(curPage) => {
            setLoading(true)
            try {
                await axios.get("https://we-safe-partner-portal-backend1.onrender.com/partners").then(res=>{
    ...
    const handleFilterSubmit=async(e) => {
        e.preventDefault()
        axios.post("https://we-safe-partner-portal-backend1.onrender.com/admin/filter",{partnerSelect,qrAssigned,qrScanData,registerDateStart,registerDateEnd, partnerId},{
            "headers":{
        ...
        console.log(partnerSelect,qrAssigned,qrScanData,registerDateStart,registerDateEnd, partnerId)
    }
            ...
            <div className='row' >
                    {!loading?
                        (newData?.map((data) => {
                            return(
                                <>
                                    <div key={data._id} className="column_admin" style={{width:'160px'}} >
                                    ...
                                    <div key={data._id} className="column_admin" style={{width:'180px'}}>
                                        ...
                                        <div className='content1'  >
                                        {
                                            ...
                                            <div style={{display:'flex',alignItems:'left',justifyContent:'center'}} >
                                                <p><span> {data.dateRegistered}</span></p>
                                            </div>
                                            ):(
                                                <div style={{display:'flex',alignItems:'left',justifyContent:'center'}} ><p><span>N/A</span></p></div>
                                            )
                                        }
                                        </div>
                                        </div>
                                    ...
                                    <div className='content1'  >
                                        {
                                            data.dob!=null?(
                                            <div style={{display:'flex',alignItems:'left',justifyContent:'center'}}  >
                                                <p><span>{JSON.stringify(data.dob).substring(1,11)}</span></p>
                                            </div>
                                            ):(
                                                <div style={{display:'flex',alignItems:'left',justifyContent:'center'}}  ><p><span> N/A</span></p></div>
                                            )
                                ...
                                <div className="card_content_admin">
                                    ...
                                    {(data?.customerQrs?.length>0)?(
                                        ...
                                        <div key={qr._id} >
                                            <div >{qr.qrId} - <span> {qr.qrPin}</span> <span> 
                                                <MdDeleteForever 
                                                onClick={e=>
                                                {   e.preventDefault()
                                                ...
                                                style={{verticalAlign:'bottom',color:'red',fontSize:'1rem'}} /> </span>
                                            </div>
                                        </div>
                                            ))
                                        ):(
                                    ...
                                    <div className='content1' style={{display:'flex',justifyContent:'center'}}  >
                                        ...
                                        style={{verticalAlign:'bottom',color:'red',fontSize:'1.3rem'}} 
                                        />
                                        <MdQrCodeScanner 
                                             onClick={() => {
                                                setCustId(data._id)
                                            ...
                                            style={{verticalAlign:'bottom',fontSize:'1.3rem'}}
                                        />
                    ...
                    }
                    {
                        modal && (
                                ...
                                <MdClose onClick={toggleModal}  style={{color:'#313bac',height:'1.2rem',width:'1.3rem',marginLeft:'300px'}}  />
                                    ...
                                    <label style={{marginLeft:'10px'}} >Select Partner</label><select onChange={e => {
                                        setPartnerSelect(e.target.value)
                                        setPartnerId(e.target.children[e.target.selectedIndex].getAttribute('partner_id'))
                                    }} 
                                        className='select-long' label='groups' 
                                    >
                                        <option value="" disabled selected >All</option>
                                        {
                                            allPartners?.map((partner) => (
                                                <option key={partner._id} partner_id={partner.partnerUid} >{partner.businessName}</option>
                                            ))
                                        }
                                    </select>
