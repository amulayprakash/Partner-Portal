...
const HomePage = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        await axios
          .get(process.env.REACT_APP_BASE_URL + "/customerData")
      ...
      await axios
        .get(
          process.env.REACT_APP_BASE_URL +
            `/customerData/${pUid}/search?searchKey=${lowerCaseValue}`
        )
  ...
  const handleFilterSubmit = (e) => {
    ...
    axios
      .post(
        process.env.REACT_APP_BASE_URL + `/customerData/${pUid}/filter`,
        {
          groupSelect,
          groupAssigned,
          qrAssigned,
  ...
  const handleCustomerSubmit = (e) => {
    ...
    if (addMutliCustomers) {
      axios
        .post(
          process.env.REACT_APP_BASE_URL + "/uploadMultipleCustomers",
          { filename },
          {
            headers: {
  ...
  const csvurl =
    "https://partner-portal.netlify.app/customerDataTemplateCSV.csv";
  ...
  useEffect(() => {
    if (cookies.get("type") === "admin") navigate("/admin");
    else navigate("/home");
  }, []);

  useEffect(() => {
    if (!cookies.get("token")) setLoggedInUser(false);
    else setLoggedInUser(true);
  }, []);
  ...
  return !loggedInUser ? (
    <>
      <div>
        ...
        />
        <h2 style={{ marginTop: "15px", marginLeft: "42%" }}>
          WeSafe Partner Portal
        </h2>
      </div>
    ...
    <div>
      ...
      <div className="app__header-btns">
        <div
          style={{ paddingBottom: "25px", display: "flex", marginTop: "10px" }}
        >
          ...
          >
            Add Documents{" "}
            <span>
              <MdUploadFile style={{ marginLeft: "5px" }} />{" "}
            </span>{" "}
          </Button>
      ...
      <div>
        ...
        <TableCard
          ...
          toggleAddToGroup={toggleAddToGroup}
          addMultiDoc={addMultiDoc}
          toggleMultiDocModal={toggleMultiDocModal}
          filteredData={filteredData}
          filterDataDisplayed={filterDataDisplayed}
          setFilterDataDisplayed={setFilterDataDisplayed}
        />
          ...
          <div className="modal">
            ...
            <div className="modal-content">
              ...
              >
                ...
                <label style={{ marginLeft: "10px" }}>Groups</label>
                <select
                ...
                <label style={{ marginLeft: "10px" }}>Groups Assigned</label>
                <select
                  onChange={(e) => setgroupAssigned(e.target.value)}
                  className="select-long"
                ...
                <label style={{ marginLeft: "10px" }}>Qr Code Assigned</label>
                <select
                ...
                <label style={{ marginLeft: "10px" }}>Docs Assigned</label>{" "}
                <select
                  onChange={(e) => setdocsAssigned(e.target.value)}
                  className="select-long"
                  label="docs assigned"
                >
                  <option value="">Both</option>
